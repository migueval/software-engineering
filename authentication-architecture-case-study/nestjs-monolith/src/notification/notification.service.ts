...
export class NotificationService implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    if (host && port && user && pass) {
      ...
      this.logger.log('Nodemailer configurado con servidor SMTP de producción.');
    } else {
  ...
  async sendOtp(email: string, code: string, lang: 'es' | 'en' = 'es'): Promise<void> {
    const isEs = lang === 'es';
  ...
  async sendWelcome(email: string, name: string, lang: 'es' | 'en' = 'es'): Promise<void> {
    const isEs = lang === 'es';
